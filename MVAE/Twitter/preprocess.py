...
def get_tweets():
    ...
    for tweet in train_tweets:
        ...
        tweet = ' '.join(tweet) + '\n'
        if detect(tweet) != 'en':
            translated_train_tweets[ct] = tweet
            ct += 1
        tweet =  tweet.encode('utf-8')
        text.append(tweet)

    print("Tweets not in english:", ct)
    pkl.dump(translated_train_tweets, open('../data/translated_train_tweets.pkl', 'wb'))
    f = open('../data/train_tweets.txt', 'wb')
    f.writelines(text)
    f.close()
    
    ct = 0
    test_tweets = open('../data/test_posts_with_img.txt',encoding='utf-8').readlines()
    text = []
